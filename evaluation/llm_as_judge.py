...
def parse_args():
    ...
    # 获取CUDA_VISIBLE_DEVICES环境变量，如果不存在则使用默认值"0"
...
def llm_as_judge(args):
    ...
    if not args.use_openai:#修改了
        # 不要覆盖命令行传入的model_name_or_path，使用传入的路径
        # 仅在未指定时提供默认值
        if not hasattr(args, 'model_name_or_path') or not args.model_name_or_path:
            args.model_name_or_path = "/lpai/volumes/base-mindgpt-ali-sh-mix/luhengtong/base_models/Qwen_models/Qwen2.5/Qwen2.5-7B-Instruct"
        # 仅在未指定时设置并行参数
        if not hasattr(args, 'tensor_parallel_size') or not args.tensor_parallel_size:
            args.tensor_parallel_size=4
        if not hasattr(args, 'data_parallel_size') or not args.data_parallel_size:
