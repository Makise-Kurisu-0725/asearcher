...
def prepare_data(data_name, args, save_async=False):
    ...
    # 检查数据集目录是否存在
    dataset_dir = os.path.join(args.data_dir, data_name)
    if not os.path.exists(dataset_dir):
        raise FileNotFoundError(f"数据集目录不存在: {dataset_dir}")
    
    # 尝试加载数据文件
    jsonl_file = os.path.join(dataset_dir, f"{args.split}.jsonl")
    json_file = os.path.join(dataset_dir, f"{args.split}.json")
    
    if os.path.exists(jsonl_file):
        print(f"Loading data from {jsonl_file}")
        processes = [json.loads(ff) for ff in open(jsonl_file, "r")]
    elif os.path.exists(json_file):
        print(f"Loading data from {json_file}")
        processes = json.load(open(json_file, "r"))
    else:
        raise FileNotFoundError(f"数据文件不存在: {jsonl_file} 或 {json_file}")
