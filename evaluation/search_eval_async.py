...
def parse_args():
    ...
    # 注意：这些参数已保留以确保兼容性，但实际实现已使用api_tools.py中的函数
    parser.add_argument("--use-jina", action="store_true", help="已保留但实际实现已使用api_tools.py中的函数")
    parser.add_argument("--jina-api-key", type=str, help="已保留但实际实现已使用api_tools.py中的函数")
    ...
    if args.llm_as_judge:
        ...
        try:
            cmd = [
                ...
                # 注意：不传递use-jina和jina-api-key参数，因为实际实现已使用api_tools.py中的函数
            ]
